...
data Derivation = Derivation
    ...
    , args :: [Text]
    , env :: Map Text Text
    , mFixed :: Maybe (DSum Store.HashAlgo Hash.Digest)
    , hashMode :: HashMode
    , useJson :: Bool
    }
    deriving (Show)
...
makeStorePathName name = case Store.mkStorePathName name of
    Left err -> throwError $ ErrorCall $ "Invalid name '" <> show name <> "' for use in a store path: " <> toString err
...
writeDerivation :: (Framed e m, MonadStore m) => Store.StoreDir -> Derivation -> m Store.StorePath
writeDerivation storeDir drv@Derivation{inputs, name} = do
...
hashDerivationModulo
    Derivation
        { mFixed = undefined -- Just (Some (digest :: Hash.Digest hashType))
        , outputs
        , hashMode
        } =
        case Map.toList outputs of
            [("out", path)] ->
                pure
                    ...
                    <> ":"
                    <> undefined -- (Store.algoName @hashType)
                    <> ":"
...
unparseDrv Derivation{..} =
  ...
  where
    produceOutputInfo (outputName, outputPath) =
        let prefix = if hashMode == Recursive then "r:" else mempty
         in parens
                $ (s <$>)
                $ ([outputName, outputPath] <>)
                $ maybe
                    [mempty, mempty]
                    ( \(Some (digest :: Hash.Digest hashType)) ->
...
derivationParser = do
  ...
  where
    ...
    parseFixed :: [(Text, Text, Text, Text)] -> (Maybe (DSum Hash.Digest), HashMode)
